    ...
    in
    {
      ...
      assertions = [
        ...
        }
        {
          assertion =
            let
              portsPerServer = lib.mapAttrsToList (
                  ...
                  mcServerFinalListeningPort =
                    if
                      ...
                      &&
                        ...
                        (
                          (serverConf.lazymc.config ? advanced)
                          && (serverConf.lazymc.config.advanced ? rewrite_server_properties)
                          && serverConf.lazymc.config.advanced.rewrite_server_properties
                          ||
        ...
        let
          ...
          getTCPPorts =
            ...
            # lazymc enabled, using public lazymc port
              ...
              in
              [ (lib.toInt (parsePort lazymcPublicAddr)) ]
            # lazymc disabled, open MC server port
            else
      ...
      systemd.sockets = pipe servers [
        (filterAttrs (
          name: server: !server.lazymc.enable && server.managementSystem.systemd-socket.enable
        )) # Only for non-lazymc systemd-socket
        (mapAttrs' (
          name: server: {
      ...
      systemd.services = mapAttrs' (
        ...
        let
          ...
          lazymcTomlGenerated =
            let
              ...
              effectiveLazymcInternalServerAddress =
                ...
                # Lazymc targets the port we configured for the Minecraft server
                else
          ...
          ExecStartPost = lib.optionalString (!conf.lazymc.enable) (
            getExe (
              pkgs.writeShellApplication {
        ...
        in
        {
          ...
          value = {
            description =
              "Minecraft Server ${name}" + (if conf.lazymc.enable then " (managed by lazymc)" else "");
